...
#    model.num_res_blocks = 12 # Number of residual blocks
...
def get_config():
    ...
    model.nonlinearity = 'swish'
    model.nf = 512  
    model.num_res_blocks = 16  # Can go deeper with residuals
    ...
    model.A_matrix = [[1.0, 0.8], [0.3, 1.2]]
    model.epsilon = 0.5
    
    # Optimization - CONSERVATIVE for stability
    config.optim = optim = ml_collections.ConfigDict()
    optim.weight_decay = 1e-5
    optim.optimizer = 'Adam'
    optim.lr = 5e-4  # Conservative starting LR
    optim.beta1 = 0.9
    optim.eps = 1e-8
    optim.warmup = 5000
    optim.grad_clip = 1.0
